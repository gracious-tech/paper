...
export interface TypographyConfig {
    ...
    // indent: every poetry level shifts one step left, so a first-level line (#q(1)) sits flush
    // at the margin like normal text rather than permanently indented. Prose paragraphs keep
    // their normal first-line indent (see gen_passage_inner in content_passage.ts)
